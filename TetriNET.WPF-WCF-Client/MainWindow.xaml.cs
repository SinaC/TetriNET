...
namespace TetriNET.WPF_WCF_Client
{
    ...
    public partial class MainWindow : Window
    {
        ...
        public MainWindow()
        {
            ...
            _client = new Client.Client(callback => new WCFProxy.WCFProxy(callback, baseAddress), CreateTetrimino, () => new Board(12, 22));
        ...
        private void OnPlayerLeft(int playerId, string playerName)
        {
            ...
            {
                OpponentGrid grid = GetOpponentGrid(playerId);
        ...
        private void OnPlayerRegistered(bool succeeded, int playerId)
        {
            if (succeeded)
            {
                ...
                Inventory.Client = _client;
                InGameMessages.Client = _client;
            }
            else
            {
        ...
        private OpponentGrid GetOpponentGrid(int playerId)
        {
            if (playerId == _playerId)
                return null;
